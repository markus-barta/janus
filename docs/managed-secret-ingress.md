...
- `POST /managed-service/setup/step-up` — require the authenticated
  ...
  strict CSRF, and a body containing only the CSRF token and intent reference.
...
a signed flow cookie containing only the intent reference, a one-way human
session reference, the state hash, and timestamps.
...
intent and human-session reference, and expires no later than two minutes after
the asserted authentication time. Logout and clean auth reset clear all flow,
proof, and managed-login cookies.
...
same-origin headers, CSRF token, signed step-up proof, session binding, and
intent reference. It then durably consumes the signed setup intent and replay
nonce before reading the first value byte.
